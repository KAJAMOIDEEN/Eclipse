...
public class intermediateOperations {
	...
	static void map(List<String> ls) { // function in functional interface
		ls.stream().map(String::toUpperCase)// transform the element i.e- takes one data and convert into another
				.forEach(System.out::println);
	}

	static void flatMap(List<List<String>> lsofLs) {
		lsofLs.stream().flatMap(Collection::stream) // to flat the list of list element
				.map(String::toLowerCase).forEach(System.out::println);

	}

	static void peek(List<Integer> ls) {
		ls.stream().map(i -> i * 2).peek(System.out::println) // will not modify the list rather than printing the
																// element
	...
	static void sorted(List<String> strLs) {
		strLs.stream().sorted(Comparator.reverseOrder()).forEach(System.out::println);
	}

	static void skip(List<Integer> strLs) {
		strLs.stream().skip(5).forEach(System.out::println);
	}

	static void limit(List<Integer> strLs) {
		strLs.stream().limit(1).forEach(System.out::println);
	}
